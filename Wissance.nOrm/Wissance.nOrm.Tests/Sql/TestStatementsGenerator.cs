...
namespace Wissance.nOrm.Tests.Sql
{
    ...
    {
        [Theory]
        [InlineData("id", WhereComparison.Equal, false, false, new[] {"2"}, "WHERE id = 2 ")]
        [InlineData("id", WhereComparison.Equal, true, false, new[] {"2"}, "WHERE id NOT  = 2 ")]
        [InlineData("id",WhereComparison.NotEqual, false, false, new[] {"2"}, "WHERE id != 2 ")]
        [InlineData("name", WhereComparison.Equal, false, true, new[] {"sensor_1"}, "WHERE name = 'sensor_1' ")]
        [InlineData("value",WhereComparison.GreaterOrEqual, false, false, new[] {"50"}, "WHERE value >= 50 ")]
        [InlineData("code",WhereComparison.In, false, false, new[] {"50", "75", "90"}, "WHERE code IN (50,75,90) ")]
        [InlineData("code",WhereComparison.In, true, false, new[] {"50", "75", "90"}, "WHERE code NOT  IN (50,75,90) ")]
        [InlineData("city",WhereComparison.In, false, true, new[] {"Yekaterinburg", "Perm", "Krasnoyarsk"}, "WHERE city IN ('Yekaterinburg','Perm','Krasnoyarsk') ")]
        [InlineData("value",WhereComparison.Between, false, false, new[] {"75", "90"}, "WHERE value BETWEEN 75 AND 90 ")]
        [InlineData("code",WhereComparison.Between, false, true, new[] {"0001", "0101"}, "WHERE code BETWEEN '0001' AND '0101' ")]
        ...
        public void TestGenerateMultipleWhereCondition()
        {
            ...
            string expectedStatement = "WHERE city IN ('Yekaterinburg','Perm')   AND  age > 18   AND  age <= 60   OR  name NOT  IN ('Ivan','Petr') ";
